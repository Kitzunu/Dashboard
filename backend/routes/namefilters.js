...
router.get('/', requireGMLevel(2), async (req, res) => {
  try {
    const [profanity] = await req.charPool.query('SELECT name FROM profanity_name ORDER BY name');
    const [reserved]  = await req.charPool.query('SELECT name FROM reserved_name ORDER BY name');
...
router.post('/:type', requireGMLevel(2), async (req, res) => {
  ...
  try {
    await req.charPool.query(`INSERT INTO ${table(type)} (name) VALUES (?)`, [name]);
...
router.delete('/:type/:name', requireGMLevel(2), async (req, res) => {
  ...
  if (!VALID_TYPES.includes(type)) return res.status(400).json({ error: 'Invalid type' });
  try {
    const [result] = await req.charPool.query(`DELETE FROM ${table(type)} WHERE name = ?`, [name]);
