...
- **Accounts** — Search by username, email, or IP; view full account detail and characters; set GM level, expansion, email, lock/unlock, reset password, mute/unmute characters, create accounts, and delete accounts. **RBAC Permissions** — view the role and effective permissions inherited from the account's security level (post-RBAC AzerothCore), inspect per-account `rbac_account_permissions` overrides, and grant/deny/revoke permissions per realm
...
- **Character Transfer** — Transfer characters between accounts or across realms; search characters by name, view GUID/race/class/level/online status; validate transfer eligibility (must be offline); search or enter target account; cross-realm transfers use pdump (dump/load with full GUID remapping); confirmation modal with audit logging
...
- **Analytics** — Historical analytics with canvas-rendered charts for player count, CPU usage, and memory usage; configurable date range presets (24h, 7d, 30d); resolution modes (raw, hourly, daily); per-realm player count filtering when multiple realms configured; summary cards showing peak players and average CPU/memory over various windows
...
- **Health Check** — System health dashboard showing database pool status (per-pool latency, free/active/total connections), Node.js system info (version, uptime, PID, platform, CPU cores, heap/RSS memory), and service status (server bridge connectivity, agent status and uptime); all-realm view showing per-realm pool health when multiple realms configured; optional auto-refresh every 10 seconds *(Administrator only)*
...
- **Multi-Realm** — Full multi-realm support via `worldservers.json`; realm selector for database-scoped pages, server selector for command-targeted pages; per-realm analytics, health checks, backup grouping, config scanning, and cross-realm character transfers; single-realm setups work with no configuration changes
